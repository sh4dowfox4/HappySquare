...
void Game::applyResolution(int resIndex, int monitorIndex) {
    if (resIndex < 0 || resIndex >= (int)resolutions.size()) {
        return;
    }
...
void Game::drawGameplay() {
    ...
    for (int x = 0; x <= WORLD_WIDTH; x += 100) {
        DrawLine(x, 0, x, WORLD_HEIGHT, DARKGRAY);
    }
    for (int y = 0; y <= WORLD_HEIGHT; y += 100) {
        DrawLine(0, y, WORLD_WIDTH, y, DARKGRAY);
    }
...
void Game::updateMenu(float deltaTime) {
    ...
    if (CheckCollisionPointRec(GetMousePosition(), btnPlay)) {
        hoveredButton = 0;
    } else if (CheckCollisionPointRec(GetMousePosition(), btnSettings)) {
        hoveredButton = 1;
    } else if (CheckCollisionPointRec(GetMousePosition(), btnExit)) {
        hoveredButton = 2;
    } else {
        hoveredButton = -1;
    }
...
void Game::drawMenu() {
	...
	DrawText("Music created by Toby Fox", 10, sh - 30, 16, GRAY);
}
...
void Game::updateGameOver(float deltaTime) {
    ...
    if (CheckCollisionPointRec(GetMousePosition(), btnNewGame)) {
        hoveredButton = 0;
    } else if (CheckCollisionPointRec(GetMousePosition(), btnMainMenu)) {
        hoveredButton = 1;
    } else {
        hoveredButton = -1;
    }
...
void Game::updatePaused(float deltaTime) {
    ...
    if (CheckCollisionPointRec(GetMousePosition(), btnResume)) {
        ...
        hoveredButton = 2;
    } else if (CheckCollisionPointRec(GetMousePosition(), btnExitGame)) {
        hoveredButton = 3;
    } else {
        hoveredButton = -1;
    }
...
void Game::drawSettings() {
    ...
    DrawText(resName, rectRes.x + (rectRes.width - MeasureText(resName, 20))/2,
            rectRes.y + 10, 20, hoverRes ? WHITE : BLUE);
    ...
    if (monitorCount > 1) {
        ...
        DrawText(monName, rectMon.x + (rectMon.width - MeasureText(monName, 20))/2,
                rectMon.y + 10, 20, hoverMon ? WHITE : BLUE);
    }
...
void Game::drawNeonButton(Rectangle rec, const char* text, bool isHovered, Color baseColor) {
    ...
    if (isHovered) {
        DrawRectangleRec((Rectangle){rec.x - 2, rec.y - 2, rec.width + 4, rec.height + 4},
                        Fade(baseColor, 0.3f));
    }
